...
interface ExamSettings {
  ...
  multipleChoiceScoring: 'all-or-nothing' | 'partial' | 'partial-with-penalty';
  penaltyValue: number;
}
...
const CreateExamModal: React.FC<CreateExamModalProps> = ({
  ...
  const [examSettings, setExamSettings] = useState<ExamSettings>({
    ...
    multipleChoiceScoring: 'all-or-nothing', // Ensure it's present
    penaltyValue: 0 // Ensure it's present
  });
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-700">שיטת ניקוד לשאלות רב-ברירה</h3>
                <select
                  value={examSettings.multipleChoiceScoring}
                  onChange={(e) => setExamSettings({ 
                    ...examSettings, 
                    multipleChoiceScoring: e.target.value as 'all-or-nothing' | 'partial' | 'partial-with-penalty'
                  })}
                  className="w-full p-2 border border-gray-300 rounded-md"
                >
                  <option value="all-or-nothing">הכול או כלום</option>
                  <option value="partial">ללא קנס</option>
                  <option value="partial-with-penalty">כולל הפחתה</option>
                </select>
                {examSettings.multipleChoiceScoring === 'partial-with-penalty' && (
                  <input
                    type="number"
                    placeholder="קנס על תשובה שגויה"
                    value={examSettings.penaltyValue || ''}
                    onChange={(e) => setExamSettings({ 
                      ...examSettings, 
